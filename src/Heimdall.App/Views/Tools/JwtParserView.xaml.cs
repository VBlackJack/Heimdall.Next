...
public partial class JwtParserView : UserControl, IToolView
{
    ...
    private void ApplyExpirationTheme() { if (!_vm.IsExpirationVisible || _vm.ExpirationStatus == JwtExpirationStatus.InvalidClaim) { return; } var brush = ResolveBrush(_vm.ExpirationStatus switch { JwtExpirationStatus.Expired => "ErrorBrush", JwtExpirationStatus.Valid => "SuccessBrush", _ => "TextSecondaryBrush" }); ExpirationBorder.Background = CreateOverlayBrush(brush.Color); ExpirationBorder.BorderBrush = brush; TxtExpiration.Foreground = brush; }
    private void UpdateHelpText() => TxtHelpContent.Text = (_localizer?["ToolHelpJWT"] ?? "ToolHelpJWT").Replace("\\n", "\n", StringComparison.Ordinal);
    private SolidColorBrush ResolveBrush(string key) => FindResource(key) as SolidColorBrush ?? new SolidColorBrush(key == "SuccessBrush" ? System.Windows.Media.Color.FromRgb(0, 180, 0) : key == "ErrorBrush" ? System.Windows.Media.Color.FromRgb(255, 0, 0) : System.Windows.Media.Color.FromRgb(160, 160, 160));
    private static SolidColorBrush CreateOverlayBrush(System.Windows.Media.Color color) { var brush = new SolidColorBrush(System.Windows.Media.Color.FromArgb(40, color.R, color.G, color.B)); brush.Freeze(); return brush; }
}
