...
{
    private readonly UuidGeneratorViewModel _vm;
    private LocalizationManager? _localizer;
    private bool _disposed;
    public UuidGeneratorView()
    {
        InitializeComponent();
        _vm = new UuidGeneratorViewModel((Application.Current as App)?.Services?.GetService<IUuidGeneratorToolService>());
        DataContext = _vm;
        TxtCount.KeyDown += OnCountKeyDown;
    }
    public void Initialize(ToolContext? context, LocalizationManager? localizer)
    {
        ...
        _vm.MarkInitialized();
        _ = Dispatcher.BeginInvoke(DispatcherPriority.Loaded, () => BtnGenerate.Focus());
    }
    ...
    private void OnLocaleChanged(string _) { _vm.OnLocaleChanged(); if (HelpPanel.Visibility == Visibility.Visible) { UpdateHelpText(); } }
    private void UpdateHelpText() => TxtHelpContent.Text = L("ToolHelpUUID").Replace("\\n", "\n", StringComparison.Ordinal);
