...
/// <paramref name="AuthenticationLevel"/> value 2 attempts server authentication and warns on failure;
/// value 0 imposes no server-authentication requirement. The same numeric values apply to the
/// .rdp <c>authentication level:i:</c> field and to
...
/// <summary>
/// Resolves RDP authentication settings from the NLA toggle.
...
public static class RdpAuthenticationResolver
{
    ...
    /// </summary>
    public static RdpAuthenticationSettings Resolve(bool nlaEnabled)
    {
        return nlaEnabled
            ? new RdpAuthenticationSettings(2, true)
            : new RdpAuthenticationSettings(0, false);
    }
}
