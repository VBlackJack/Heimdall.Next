...
public sealed class VncHandlerValidationTests
{
    [Fact]
    public async Task ConnectAsync_RejectsOutOfRangePort()
    {
        VncHandler handler = CreateHandler();
    ...
    {
        VncHandler handler = CreateHandler();
        ServerProfileDto server = CreateServer("   ", 5900);
        AppSettings settings = new AppSettings();
    ...
    private static VncHandler CreateHandler()
    {
        return new VncHandler(new ConnectionStateMachine(), new LocalizationManager());
    }
